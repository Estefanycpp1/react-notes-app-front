...
export const NoteProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchNotes = async () => {
    ...
    try {
      const response = await axios.get<Note[]>('https://react-notes-app-back-production-c147.up.railway.app/getnofoldernotes', {
  ...
  const toggleFavorite = async (noteId: string) => {
    ...
    try {
      await axios.put(`https://react-notes-app-back-production-c147.up.railway.app/togglefavorite/${noteId}`, null, {
        headers: { Authorization: `Bearer ${token}` },
      });
